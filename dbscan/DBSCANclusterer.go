...
func (this *DBSCANClusterer) Cluster(data []ClusterablePoint) [][]ClusterablePoint {
	...
	var (
		...
		neighborhoodMap []*ConcurrentQueueInsertOnly
	)
...
func (this *DBSCANClusterer) BuildNeighborhoodMap(data []ClusterablePoint) []*ConcurrentQueueInsertOnly {
	var (
		dataSize  = len(data)
		result    = make([]*ConcurrentQueueInsertOnly, dataSize)
		...
		fn = func(start int) {
			...
			if result[start] == nil {
				result[start] = NewConcurrentQueueInsertOnly()
			}
			...
			for i := start + 1; i < dataSize && data[i].GetPoint()[this.SortDimensionIndex] <= headDimV; i += 1 {
				...
				if this.CalcDistance(headV, x.GetPoint()) <= this.eps2 {
					...
					if result[i] == nil {
						result[i] = NewConcurrentQueueInsertOnly()
					}
